...
import type { InputSwapProps } from "../../types/types";
...
const InputSwap = ({
  ...
  const handleAmountChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (inputValue.match(/^\d*\.?\d*$/)) {
      setAmount(inputValue); // Set the numeric value in the state
      handleInputChange(event);
    } else {
      setAmount(''); // Handle the case where the input is not a valid number
    }
  };
  ...
  // useEffect(() => {
  //     const documentClickHandler = (e: Event) => handleClickOutside(e as MouseEvent);
  ...
  return (
    <div className="border-blue relative flex min-h-[72px] w-full cursor-pointer flex-col items-center justify-between divide-y rounded-xl border-2 bg-[#fafafa] px-3 py-1 md:flex-row md:divide-x md:divide-y-0">
      ...
      <div className="w-full pl-2 pt-3 text-[#9d9d9d] md:w-2/5">
        <p className="font-medium text-[#9d9d9d]">You send</p>
        <input
